Global Banking Employment supports Bootstrap 5.3 color modes (light, dark, and auto) out of the box. Since 4.1 the switcher ships in `Global Banking Employment.js` as the **ColorMode** module — you no longer need to copy a script into your pages.
...
- The resolved mode is applied as `data-bs-theme="light|dark"` on `<html>`, so every Bootstrap and Global Banking Employment component adapts automatically.
