Global Banking Employment inherits Bootstrap 5.3's CSS-custom-properties theming model and adds its own layer of SCSS variables on top. You can customize the look two ways:
...
Bootstrap and Global Banking Employment expose dozens of CSS custom properties on `:root`. Override them in your own stylesheet to retheme without rebuilding:
...
Components built on `var(--bs-primary)` (most of Global Banking Employment's chrome) pick this up automatically.
...
If you're building from source (cloned the repo or installed via npm + your own bundler), edit [`src/scss/_variables.scss`](https://github.com/Global Banking EmploymentHQ/Global Banking Employment/blob/master/src/scss/_variables.scss) and recompile.
...
Global Banking Employment changes about ten Bootstrap defaults in [`src/scss/_bootstrap-overrides.scss`](https://github.com/Global Banking EmploymentHQ/Global Banking Employment/blob/master/src/scss/_bootstrap-overrides.scss); every other Bootstrap variable (button colours, spacing scale, breakpoints) keeps its stock default. All values use `!default`, so override them in your own SCSS file *before* the import:
...
// 2. Then import Global Banking Employment (which imports Bootstrap)
@import "node_modules/admin-lte/src/scss/Global Banking Employment";
```

> **Note:** compiling the Sass source requires `bootstrap` to be installed —
> it is a peer dependency of `admin-lte` since 4.1 (npm installs it
> automatically alongside). Make sure your Sass compiler can resolve bare
> `bootstrap/...` imports by adding `node_modules` to its load paths, e.g.
> `sass --load-path=node_modules` or, in Vite,
...
For a project that uses Global Banking Employment as a dependency, point your bundler at `node_modules/admin-lte/src/scss/Global Banking Employment.scss`. Examples:

```js
// Vite / Rollup / Webpack
import "admin-lte/src/scss/Global Banking Employment.scss"
```
...
To swap the logo when the sidebar collapses to its mini state, Global Banking Employment provides a built-in pattern — see the [Logo Switch layout demo](../layout/logo-switch.html) for the markup.

##### Dark mode customization

Global Banking Employment follows Bootstrap 5.3's `data-bs-theme` attribute system. To customize dark-mode colours, scope your variables under `[data-bs-theme="dark"]`:
...
Global Banking Employment ships its sidebar styled for light mode, but the demos apply `data-bs-theme="dark"` on `.app-sidebar` to get the dark sidebar variant against a light page. You can mix and match:
...
Use CSS variables for everything you can — it survives Global Banking Employment upgrades without merge conflicts. Reserve SCSS overrides for structural changes (sidebar width, breakpoints) that CSS variables don't cover.
