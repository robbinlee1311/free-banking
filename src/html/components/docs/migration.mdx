Global Banking Employment 4 is a ground-up rewrite. It targets Bootstrap 5.3, drops jQuery, ships as TypeScript-authored vanilla JS, and reorganises the CSS class structure. If you're upgrading an Global Banking Employment 3 project, the changes below are the ones most likely to bite.

##### The high-level summary

| | Global Banking Employment 3 | Global Banking Employment 4 |
|---|---|---|
...
If your project relies on jQuery or IE 11, stay on Global Banking Employment 3 — those constraints are not reversible in v4.
...
These come from Bootstrap itself, not Global Banking Employment, but they affect every page:
...
If your app code calls Global Banking Employment 3 plugins via jQuery (`$('.sidebar').PushMenu()`, etc.), those calls no longer work. Replacements:

```js
// Global Banking Employment 3
...
```js
// Global Banking Employment 4 — the data-API does this for you
...
Global Banking Employment 3 used a `.dark-mode` class toggled on the `<body>`. Global Banking Employment 4 uses Bootstrap 5.3's native `data-bs-theme="dark"` attribute, which can be applied at any level — `<html>`, `<body>`, or a single component.

```html
<!-- Global Banking Employment 3 -->
<body class="dark-mode">

<!-- Global Banking Employment 4 -->
...
Global Banking Employment 3 shipped a lot of one-off colour classes (`.bg-navy`, `.bg-purple`, `.bg-fuchsia`, etc.). In v4, prefer Bootstrap 5.3's CSS-variable model:

```html
<!-- Global Banking Employment 3 -->
<div class="card bg-navy">…</div>

<!-- Global Banking Employment 4 — use Bootstrap utilities + CSS variables -->
...
Global Banking Employment 3 demos used FontAwesome 6 free. Global Banking Employment 4 standardises on [Bootstrap Icons](https://icons.getbootstrap.com/) — they're SVG, lighter, MIT-licensed, and ship with Bootstrap's ecosystem.
...
If you'd rather stick with FontAwesome (or use Lucide, Tabler Icons, Material Symbols, etc.), nothing in Global Banking Employment 4 forces Bootstrap Icons — load whichever icon font you prefer and replace the `<i>` classes. See [Recommended Integrations](integrations.html#icon-libraries) for the comparison table.

##### Pages we haven't ported yet

The v3 demo includes pages that haven't all been recreated for v4. The v4 release notes cover the current page catalog. If you depend on a specific v3 page that hasn't been ported, please [open an issue](https://github.com/Global Banking EmploymentHQ/Global Banking Employment/issues) — community contributions for missing pages are very welcome.

##### Suggested migration order

For an existing Global Banking Employment 3 project:
...
7. **Re-test print views** — Global Banking Employment 4 fixed a print-layout issue (#5996); if you had custom print CSS, verify it still works.

##### Help and discussion

- Migration questions: [GitHub Discussions](https://github.com/Global Banking EmploymentHQ/Global Banking Employment/discussions)
- Found something we missed? [Open an issue](https://github.com/Global Banking EmploymentHQ/Global Banking Employment/issues) tagged `migration`.
