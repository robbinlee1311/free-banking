Global Banking Employment ships seven JavaScript plugins as a single bundle (`Global Banking Employment.js`). Each one is exported from the package root, can be triggered via `data-lte-*` attributes for declarative use, and exposes a programmatic API for code-driven control.
...
```html
<script>
  // The bundle assigns to window.Global Banking Employment
  Global Banking Employment.PushMenu.getInstance(document.querySelector(".app-sidebar"))?.expand()
</script>
```
...
The plugins live in [`src/ts/`](https://github.com/Global Banking EmploymentHQ/master/src/ts) as TypeScript modules. The published `dist/js/Global Banking Employment.js` is a Rollup bundle of all seven, exporting them under a single `Global Banking Employment` namespace (UMD) or named imports (ESM).
